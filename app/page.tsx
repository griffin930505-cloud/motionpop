...
function posToCanvas(pos:TextPos,w:number,h:number){const padX=w*.07,padY=h*.09;const col=pos[1],row=pos[0];return{x:col==='l'?padX:col==='r'?w-padX:w/2,y:row==='t'?padY:row==='b'?h-padY:h/2,align:(col==='l'?'left':col==='r'?'right':'center') as CanvasTextAlign}}
...
export default function Home(){
 ...
 const fileRef=useRef<HTMLInputElement>(null); const videoRef=useRef<HTMLInputElement>(null); const dragRef=useRef<{x:number;y:number;ox:number;oy:number}|null>(null);
 const r=useMemo(()=>ratios.find(x=>x.id===ratio)!,[ratio]);
 ...
 const generate=async()=>{
  ...
  try{
   ...
   const gif=GIFEncoder();const fps=effect==='none'?1:15,frames=effect==='none'?1:Math.max(15,Math.round(seconds*fps));
   for(let i=0;i<frames;i++){
    const t=frames===1?0:i/(frames-1),loop=Math.sin(t*Math.PI*2),ping=Math.sin(t*Math.PI);
    ctx.clearRect(0,0,outW,outH);ctx.fillStyle='#0b0c10';ctx.fillRect(0,0,outW,outH);
    const base=Math.max(outW/img.width,outH/img.height);let scale=base,dx=imageOffset.x/100*outW,dy=imageOffset.y/100*outH,rot=0;
    ...
    if(imageBorder!=='none'){ctx.lineWidth=Math.max(3,fs*.11);ctx.strokeStyle=imageBorder==='white'?'#fff':'#000';ctx.strokeText(label,x,y)}ctx.fillStyle=imageFontColor;ctx.fillText(label,x,y);
    const data=ctx.getImageData(0,0,outW,outH);const palette=quantize(data.data,256);const index=applyPalette(data.data,palette);gif.writeFrame(index,outW,outH,{palette,delay:effect==='none'?1000:1000/fps});await new Promise(res=>setTimeout(res,0));
   }
